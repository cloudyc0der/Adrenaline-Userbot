...
if [[ -d "AdrenalineKiller-Userbot" ]]; then
  cd AdrenalineKiller-Userbot
elif [[ -f ".env.dist" ]] && [[ -f "main.py" ]] && [[ -d "modules" ]]; then
  :
else
  git clone https://github.com/cloudyc0der/Dragon-Userbot || exit 2
  cd AdrenalineKiller-Userbot || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  echo "It seems that AdrenalineKiller Userbot is already installed. Exiting..."
  exit
fi
...
case $db_type in
  1)
    ...
    read -r -p "> " db_url
    db_name=AdrenalineKiller_Userbot
    db_type=mongodb
    ;;
  2)
...
case $install_type in
  1)
    ...
    echo
    echo "============================"
    echo "Great! AdrenalineKiller-Userbot installed successfully and running now!"
...
[Unit]
Description=Service for AdrenalineKiller Userbot
...
EOL
  ...
  *)
    echo
    echo "============================"
    echo "Great! AdrenalineKiller Userbot installed successfully!"
