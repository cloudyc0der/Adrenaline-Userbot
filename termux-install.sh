...
if [[ -d "Adrenaline-Userbot" ]]; then
  cd Adrenaline-Userbot
elif [[ -f ".env.dist" ]] && [[ -f "main.py" ]] && [[ -d "modules" ]]; then
  :
else
  git clone https://github.com/cloudyc0der/Adrenaline-Userbot || exit 2
  cd Adrenaline-Userbot || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  echo "It seems that Adrenaline-Userbot is already installed. Exiting..."
  exit
fi
...
echo
echo "============================"
echo "Great! Adrenaline-Userbot installed successfully!"
